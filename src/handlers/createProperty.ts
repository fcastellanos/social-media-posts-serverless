...
export const handler = async (event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2> => {
  try {
    ...
    if (body.latitude === undefined) return badRequest('Missing latitude');
    if (body.longitude === undefined) return badRequest('Missing longitude');

    const title = body.title || body.name;
    const latitude = body.latitude;
    const longitude = body.longitude;

    const prop = await repo.createProperty({ title, address: body.address, latitude, longitude, metadata: body.metadata });
